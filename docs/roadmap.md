...
| 20 | Wave metrics dashboard | B | 2.5h | **RENAMED FROM** "Wave parallelism metrics". Distinguish from Wave ETA calculation (Shipped). Collect wave execution stats (wait time, utilization) for UI display. **UPDATED**: JSON schema follows convention: {wave_id, avg_wait_sec, parallelism_factor, timestamp}. Dashboard implementation deferred to v1.1+. | `scripts/stats.sh` outputs wave-level metrics (avg wait, parallelism factor), exports to JSON for future dashboard consumption |
...
**Item #20 update**: Added JSON schema convention clarification to prevent scope creep (dependency clarification from feasibility analysis).

**Wave metrics scope clarification**: Item #20 (Wave metrics dashboard) is distinct from "Wave ETA calculation (Shipped)". The shipped ETA calculation provides the *algorithm* for estimating completion time. Item #20 adds *reporting infrastructure* (stats collection, JSON export, dashboard UI) for visualizing wave performance. No overlap.
...
| 24 | Execution_log effort tracking | A | 2.5-3h | **REVISED from 2h** (feasibility: backward compatibility for 60+ existing logs adds complexity). Enhance execution_log.yaml schema to track actual vs. planned effort. Includes validation updates + backward compatibility for cmd_030-089 logs. | `execution_log.yaml` includes `effort_hours` field (optional), retrospector logs actual vs. planned hours, `scripts/stats.sh` summarizes monthly variance, validation allows optional field |
