...
- **パーミッションによる安全性** — Claude Code標準のパーミッションシステムが全サブエージェントに適用される。`permission-guard` プラグインが提供するPermissionRequest hookにより、scripts/ 配下のpython3/bash/shスクリプト実行を8段階検証パイプラインで動的に承認する。インストール: `/plugin install permission-guard@skaji18-plugins`。`--dangerously-skip-permissions` は不要。
...
│   └── permission-config.yaml # パーミッションフック設定（Layer 2: プロジェクト設定）
├── templates/
...
- **フォアグラウンド並列のみ** — `run_in_background: true` はClaude Codeの既知バグ（MCP利用不可、output_file 0バイト、通知未発火）のため使用しない。全タスクをフォアグラウンド並列で実行する。
