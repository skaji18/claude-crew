...
### Removed
- **stats.sh, analyze_patterns.sh, patterns.md** — Execution log stats and pattern mining removed; cmd_128 analysis showed selection bias in model comparisons and coder CV=97-113% makes ETA unreliable; Wave ETA calculation and decomposer Historical Patterns (W4) section also removed
...
### Added
- **Layer 1 Mutation Keyword Detection** — Decomposer scans request.md for mutation keywords (`challenge assumptions`, `find flaws`/`red team`, `explore alternatives`, `critical challenges` + Japanese equivalents); injects strategy-specific instructions into task_N.md Details section; 4 strategies (assumption_reversal, adversarial_review, alternative_exploration, comprehensive_mutation); complementary to Layer 0 Self-Challenge (cmd_129)
