...
- **Permission-based safety** — Claude Code's built-in permission system applies to all sub-agents, with a PermissionRequest hook provided by the `permission-guard` plugin that dynamically approves python3/bash/sh execution of scripts under scripts/ directories via an 8-phase validation pipeline. Install: `/plugin install permission-guard@skaji18-plugins`. No `--dangerously-skip-permissions` required.
...
│   └── permission-config.yaml # Permission hook configuration (Layer 2: project config)
├── templates/
